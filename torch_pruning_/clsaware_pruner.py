...
class ClassAwarePruner(MetaPruner):
    def set_clswise_storage(self, num_classes):
        # self.cls_weight = cls_weight
        self.groups_imps = [{} for cls in range(num_classes)]
        self.counter = [{} for cls in range(num_classes)]

    def store_importance(self, cls_id: int):
        for i, group in enumerate(self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types)):
            if self._check_pruning_ratio(group):  # check pruning ratio
                ...
                if i not in self.groups_imps[cls_id]:
                    self.groups_imps[cls_id][i] = imp
                    self.counter[cls_id][i] = 1
                else:
                    self.groups_imps[cls_id][i] += imp
                    self.counter[cls_id][i] += 1

        # print(self.counter_many[0])
    ...
    def prune_local(self) -> typing.Generator:
        ...
        for i, group in enumerate(self.DG.get_all_groups(ignored_layers=self.ignored_layers,
            ...
            if self._check_pruning_ratio(group):  # check pruning ratio
                ...
                # if num_type == 'many':
                imp = []
                for cls in range(len(self.groups_imps)):
                    if i in self.counter[cls]:
                        cls_imp = self.groups_imps[cls][i] / (self.counter[cls][i])
                        imp.append(cls_imp)
                # print(len(imp), len(self.cls_weight))
                imp = torch.stack(imp)
                # print(imp.shape)
                # assert len(self.cls_weight) == len(imp)
                # imp = imp.dot(1 - self.cls_weight).sum() / (len(self.cls_weight) - 1)
                # imp = (imp * (1 - self.cls_weight).unsqueeze(1)).sum(dim=0) / (len(self.cls_weight) - 1)
                imp = imp.mean(dim=0)
                # print(imp)
    ...
    def prune_global(self) -> typing.Generator:
        ...
        for i, group in enumerate(self.DG.get_all_groups(ignored_layers=self.ignored_layers,
            ...
            if self._check_pruning_ratio(group):
                ...
                imp = torch.stack(imp)
                # imp_mean = imp.mean(dim=0)
                # imp_std = imp.std(dim=0)
                # print('group {} \n mean: {} \n std:{}'.format(i, imp_mean, imp_std.mean()))

                # assert len(self.cls_weight) == len(imp)

                # imp = (imp * (1 - self.cls_weight).unsqueeze(1)).sum(dim=0) / (len(self.cls_weight) - 1)
                # imp = (imp * (self.cls_weight).unsqueeze(1)).sum(dim=0)
                # imp = imp.sum(dim=0) / (len(self.cls_weight))
                imp = imp.mean(dim=0)
                # print('imp: ', imp)
    ...
    # def step(self, interactive=False, num_type='many')-> typing.Union[typing.Generator, None]:
    #     self.current_step = 1
    ...
    #         return pruning_method(num_type=num_type)
    #     else:
